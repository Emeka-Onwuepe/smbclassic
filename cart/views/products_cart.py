...
class Product_Cart_Treeview:
    add_to_cart = None

    def __init__(self,frame,grand_total):
        self.frame = frame
        self.grand_total = grand_total
        self.product_frame = ttkb.LabelFrame(self.frame,width=1360,text='Products Cart')
        ...
        self.tree = ttkb.Treeview(self.product_frame,columns=self.columns,
                                 ...
                                 show='headings')
        self.tree.pack()
        controls = ttkb.LabelFrame(self.product_frame,text='controls')
        controls.pack()
        ...
        Product_Cart_Treeview.add_to_cart = self.add_data
    ...
    def updateCart(self,product=[],action='add'):
        ...
        for id in ids:
            values = self.tree.item(id,'values')
            product_total += float(values[-2])
        grand_total = update_cart('product',product_total,product,action)
        self.grand_total.config(text=grand_total)
        
    def delete_all(self):
        for record in self.tree.get_children():
            self.tree.delete(record)
        grand_total = update_cart('product',0,[],'update')
        self.grand_total.config(text=grand_total)
        self.toggle()